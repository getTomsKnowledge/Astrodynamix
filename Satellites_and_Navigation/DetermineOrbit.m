...
totalOrbits = length(night2Data.opt3satCset4.datetime);
%totalOrbits = length(opt2satCset4.datetime);
sampleSize = 10;
numberOfGroups = 3;
K = 6;

% Data accuracy tolerance:
radiusTol = 500000; %(+/- 500 kilometer tolerance)

combinCalc = factorial(sampleSize) / (factorial(sampleSize - 3)*factorial(3));
outOf = cast(combinCalc, 'int8');

azErr = zeros(3*outOf, 1);
elErr = zeros(3*outOf, 1);

%% MIXED-TIME DATA ANALYSIS:
run('get_random_od.m');


%% EARLY-TIME DATA ANALYSIS:
    ...
    for k = 1:(sampleSize - 1)
        for l = 1:sampleSize
            ...
            if ((k >= (n + 1)) && ((l >= (n + 2)) && (l >= (k + 1))))
                ...
                los1 = get_topo_los(...
                    obs1.azimuth_deg, obs1.elevation_deg, chile1);
                los2 = get_topo_los(...
                    obs2.azimuth_deg, obs2.elevation_deg, chile2);
                los3 = get_topo_los(...
                    obs3.azimuth_deg, obs3.elevation_deg, chile3);
                ...
                r1 = norm(o1out.position_m);
                r2 = norm(o2out.position_m);
                r3 = norm(o3out.position_m);
                rSum = r1 + r2 + r3;

                % Rough check against geosyncrhonous orbit radius:
                if ( (rSum > (3 * consts.R_GEO_km*1e3 + radiusTol)) || (rSum < (3 * consts.R_GEO_km*1e3 - radiusTol)) )
                    continue;
                end
...
% Get line of sight vectors:
finalLOS1 = get_topo_los(...
    final1.azimuth_deg, final1.elevation_deg, chile1);
finalLOS2 = get_topo_los(...
    final2.azimuth_deg, final2.elevation_deg, chile2);
finalLOS3 = get_topo_los(...
    final3.azimuth_deg, final3.elevation_deg, chile3);
...
%    fprintf('\n\nnope, bad beginning dataset.\n\n');
    ...
    goodSample = load('N33_Sample_12052022.mat');

    % Dummy observation triplet:
    ...
    % Dummy RMS:
    begRMSDistribution = goodSample.rmsDistribution;
...
try
    ...
    fprintf('\nBad early radii.  No RMS distribution obtained.\n');
    goodSample = load('N33_Sample_12052022.mat');
    begRMSDistribution = goodSample.rmsDistribution;
end

%% 2BP PROPAGATOR FOR EARLY TIMES:
...
fprintf('\nRESULTS:--->%s', beginningGraphTitle);
fprintf('\nBest Early Triplet:  ( %d, %d, %d ) [obs. no.],  Min. RMS: %0.4f deg.\n', ...
    final1.obsNumber, final2.obsNumber, final3.obsNumber, minRMS);

fprintf('Sample Size: %d observations, Time Elapsed: %0.1f seconds\n\n', ...
    sampleSize, timeOut);


%% LATE DATASET ANALYSIS:
...
try
    ...
    % Dummy RMS:
    endRMSDistribution = goodSample.rmsDistribution;
...
%% 2BP PROPAGATION OF LATE RESULT:
...
%% PROCESS RMS / RESIDUAL VALUES:

% REMOVE ALL ZERO ENTRIES:

% Azimuth (keep only non-zero error):
zeroIndicesAZ = azErr == 0;
azErr(zeroIndicesAZ) = [];

% Elevation (keep only non-zero error):
...
% Color code each of the distributions by time period:
earlyColor = [0 0.4470 0.7410]; % Blueish
lateColor = [0.8500 0.3250 0.0980]; % Orange-ish
mixedColor = [0.9290 0.6940 0.1250]; % Yellowish

% Pane 1: All orbits plotted simultaneously
figure('Name', 'Plot of All Orbits, 24-Hours');
plot3(beginningOrbit1(:,1)*1e-3, beginningOrbit1(:,2)*1e-3, beginningOrbit1(:,3)*1e-3, 'Color', earlyColor)
hold on
plot3(beginningOrbit2(:,1)*1e-3, beginningOrbit2(:,2)*1e-3, beginningOrbit2(:,3)*1e-3, 'Color', earlyColor)
plot3(beginningOrbit3(:,1)*1e-3, beginningOrbit3(:,2)*1e-3, beginningOrbit3(:,3)*1e-3, 'Color', earlyColor)
plot3(endingOrbit1(:,1)*1e-3, endingOrbit1(:,2)*1e-3, endingOrbit1(:,3)*1e-3, 'Color', lateColor)
plot3(endingOrbit2(:,1)*1e-3, endingOrbit2(:,2)*1e-3, endingOrbit2(:,3)*1e-3, 'Color', lateColor)
plot3(endingOrbit3(:,1)*1e-3, endingOrbit3(:,2)*1e-3, endingOrbit3(:,3)*1e-3, 'Color', lateColor)
plot3(orbit1FinalMixed(:,1)*1e-3, orbit1FinalMixed(:,2)*1e-3, orbit1FinalMixed(:,3)*1e-3, 'Color', mixedColor)
plot3(orbit2FinalMixed(:,1)*1e-3, orbit2FinalMixed(:,2)*1e-3, orbit2FinalMixed(:,3)*1e-3, 'Color', mixedColor)
plot3(orbit2FinalMixed(:,1)*1e-3, orbit2FinalMixed(:,2)*1e-3, orbit2FinalMixed(:,3)*1e-3, 'Color', mixedColor)
hold off;
legend('Early Times', '-', '-','Late Times', '-', '-', 'Mixed Times', '-', '-');
title('Plot of All Predicted Orbits Over 24-Hour Period');
xlabel('X, ECEF (km)');
ylabel('Y, ECEF (km)');
zlabel('Z, ECEF (km)');


% Early Data-Based Orbit Prediction
figure('Name', 'Gauss-Determined Orbits (Early, Late, Mixed)');
subplot(3,1,1)
plot3(beginningOrbit1(:,1)*1e-3, beginningOrbit1(:,2)*1e-3, beginningOrbit1(:,3)*1e-3, 'Color', 'r');
hold on;
plot3(beginningOrbit2(:,1)*1e-3, beginningOrbit2(:,2)*1e-3, beginningOrbit2(:,3)*1e-3, 'Color', 'g');
plot3(beginningOrbit3(:,1)*1e-3, beginningOrbit3(:,2)*1e-3, beginningOrbit3(:,3)*1e-3, 'Color', 'b');
title(beginningGraphTitle);
xlabel('X, ECEF (km)');
ylabel('Y, ECEF (km)');
zlabel('Z, ECEF (km)');
legend('Early R1 Prediction', 'Early R2 Prediction', 'Early R3 Prediction');
hold off;
% Late Data-Based Orbit Prediction
subplot(3,1,2);
plot3(endingOrbit1(:,1)*1e-3, endingOrbit1(:,2)*1e-3, endingOrbit1(:,3)*1e-3, 'Color', 'r');
hold on;
plot3(endingOrbit2(:,1)*1e-3, endingOrbit2(:,2)*1e-3, endingOrbit2(:,3)*1e-3, 'Color', 'g');
plot3(endingOrbit3(:,1)*1e-3, endingOrbit3(:,2)*1e-3, endingOrbit3(:,3)*1e-3, 'Color', 'b');
title(endingGraphTitle);
xlabel('X, ECEF (km)');
ylabel('Y, ECEF (km)');
zlabel('Z, ECEF (km)');
legend('Late R1 Prediction', 'Late R2 Prediction', 'Late R3 Prediction');
hold off;
% Mixed Data-Based Orbit Prediction
subplot(3,1,3);
plot3(orbit1FinalMixed(:,1)*1e-3, orbit1FinalMixed(:,2)*1e-3, orbit1FinalMixed(:,3)*1e-3, 'Color', 'r');
hold on;
plot3(orbit2FinalMixed(:,1)*1e-3, orbit2FinalMixed(:,2)*1e-3, orbit2FinalMixed(:,3)*1e-3, 'Color', 'g');
plot3(orbit2FinalMixed(:,1)*1e-3, orbit2FinalMixed(:,2)*1e-3, orbit2FinalMixed(:,3)*1e-3, 'Color', 'b');
title(titleOfMixedGraph);
xlabel('X, ECEF (km)');
ylabel('Y, ECEF (km)');
zlabel('Z, ECEF (km)');
legend('Mixed R1 Prediction', 'Mixed R2 Prediction', 'Mixed R3 Prediction');
hold off;

% Plot of Azimuth, Elevation Prediction Errors Over the Course of Observation:
figure('Name', 'Azimuth, Elevation Errors Over Time');
plot(azErr(:), 'Color', [0.4940 0.1840 0.5560]);
hold on; 
plot(elErr(:), 'Color', [0.4660 0.6740 0.1880]);
legend('Azimuth (deg.)', 'Elevation(deg.)');
xlabel('Position in Random Sample (#))');
ylabel('Angle, AZ / EL (deg.)');
hold off;

% Pane 3: RMS values over time
figure('Name', 'RMS Values Over Time');
plot(begRMSDistribution(:), 'Color', earlyColor);
hold on;
plot(endRMSDistribution(:), 'Color', lateColor);
plot(mixedRMSDistribution(:), 'Color', mixedColor);
xlabel('Position in Random Sample (#))');
ylabel('Total Angular Error (deg.)');
legend('Early RMS (deg.)', 'Late RMS (deg.)', 'Mixed RMS (deg.)');
hold off;

% Pane 4: Histograms of Distributions
figure('Name', 'Histograms of RMS Distributions (Early, Late, Mixed Times)');
subplot(3,1,1);
histogram(begRMSDistribution, 'FaceColor', earlyColor);
xlabel('Total Degree Error (deg.)');
ylabel('Count (#)');
title('Early RMS Distribution');
subplot(3,1,2);
histogram(endRMSDistribution, 'FaceColor', lateColor);
histogram(begRMSDistribution, 'FaceColor', earlyColor);
xlabel('Total Degree Error (deg.)');
ylabel('Count (#)');
title('Late RMS Distribution');
subplot(3,1,3);
histogram(mixedRMSDistribution, 'FaceColor', mixedColor);
histogram(begRMSDistribution, 'FaceColor', earlyColor);
xlabel('Total Degree Error (deg.)');
ylabel('Count (#)');
title('Mixed-Time RMS Distribution');
